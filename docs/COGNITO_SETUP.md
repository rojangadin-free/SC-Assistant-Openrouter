...
DynamoDB to `arn:aws:dynamodb:us-east-1:225119180951:table/*`, and
`ap-southeast-1` has only `StudentRecords`, `SCAssistantStores` and
`SCAssistantReports`. **`Files` and `Conversations` do not exist there**, so file
uploads and conversation history will fail once sign-in works. `AWS_REGION` also
sets `S3_BUCKET_NAME`'s region implicitly — `sc-assistant-bucket` has to exist in
`ap-southeast-1` too.
